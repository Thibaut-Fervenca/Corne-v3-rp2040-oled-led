// Classic Adafruit_GFX 5x7 fixed-space bitmap font.
// Copied here (and renamed to `oled_kfont`) so the keymap can read glyph data
// for scaled rendering without colliding with the OLED driver's own `font[]`.
// Layout: 6 bytes per character (5 columns + 1 spacing), each byte a vertical
// column of 8 pixels (bit 0 = top). Character `c` starts at oled_kfont[c * 6].
#pragma once

#include "progmem.h"

static const unsigned char oled_kfont[] PROGMEM = {
